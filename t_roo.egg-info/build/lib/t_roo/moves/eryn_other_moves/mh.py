...
class MHMove(Move):
    ...
    def propose(self, model, state):
        ...
        for (branch_names_run, inds_run) in self.gibbs_sampling_setup_iterator(
            ...
            ### As in reverse jump, also here we remove lambdas from
            ### bbh coordsinates for proposal

            lambda1_coord = coords_going_for_proposal['nsbh'][:, :, :, 11]
            #lambda2_coord = coords_going_for_proposal['nsbh'][:, :, :, 12]
            
            #coords_mod = np.delete(coords_going_for_proposal['bbh'], [11,12], axis=3)
            coords_mod = np.delete(coords_going_for_proposal['bbh'], [11], axis=3)
            coords_to_use = {}
            coords_to_use['nsbh'] = coords_going_for_proposal['nsbh']
            coords_to_use['bbh'] = coords_mod 
            ...
            ### Adding lambdas back also to bbh coords so we remember them
            ### Again separate cases for bbh and nsbh since q is a dictionary

            q_new = {}
            #coords_back_int = np.insert(q['bbh'],11,lambda1_coord,axis=3)
            #coords_back = np.insert(coords_back_int,12,lambda2_coord,axis=3)
            coords_back = np.insert(q['bbh'],11,lambda1_coord,axis=3)
            
            q_new['bbh'] = coords_back
            q_new['nsbh']=q['nsbh']
