...
class HDFBackend(Backend):
    ...
    Args:
        ...
        store_missing_leaves (double, optional): Number to store for leaves that are not
            used in a specific step. (default: ``0.0``)
    ...
    def reset(
        ...
        **info,
        ...
        with self.open("a") as f:
            ...
            g.attrs["parameter_names"] = parameter_names
    ...
    def converged(self):
        with self.open() as f:
            return f[self.name]['info'].attrs['converged'] # FIXME: This is ugly and should be nicer at some point
    ...
    def save_step(
        ...
        Args:
            ...
            pre_steps: True if we are in the within-model pre-rjmcmc sampling stage, otherwise False 
        ...
        with self.open("a") as f:
            ...
            self._check(
                state, accepted, pre_steps, rj_accepted=rj_accepted, swaps_accepted=swaps_accepted,
            )
            ...
            for name, model in state.branches.items():
                ...
                g["chain"][name][self.iteration] = model.coords #coords_in
    ...
    def set_as_converged(self,):
        with self.open("a") as f:
            f[self.name]['info'].attrs["converged"] = True
    ...
    def get_posterior_samples(self, 
        ...
        with self.open() as f:
            samples = f[self.name]['chain'][branch][ind_start:ind_end, 0, :, 0, :]
            inds = f[self.name]['inds'][branch][ind_start:ind_end, 0, :, 0]
            parameter_names = f[self.name].attrs['parameter_names']
        ...
        for j, key in enumerate(parameter_names):
            if flatten:
                posterior[key] = (samples[:, :, j].flatten())[inds]
            else:
                posterior[key] = samples[:, :, j]
                posterior[key][~inds] = np.nan

        return posterior, inds
    ...
    def get_log_likelihood(self,
        ...
        with self.open() as f:
            log_lik = f[self.name]['log_like'][ind_start:ind_end, 0, :]
            inds = f[self.name]['inds'][branch][ind_start:ind_end, 0, :, 0]
        
        if flatten:
            return log_lik[inds].flatten(), inds
        else:
            return log_lik, inds
