...
class RedBlueMove(Move, ABC):
    ...
    @classmethod
    def get_proposal(self, sample, complement, inds_block_proposal, random, gibbs_ndim=None):
        """Make a proposal
    ...
    def propose(self, model, state, inds_proposal):
        """Use the move to generate a proposal and compute the acceptance

        Args:
            model (:class:`eryn.model.Model`): Carrier of sampler information.
            state (:class:`State`): Current state of the sampler.
            inds_proposal ('all' or list): list of indices corresponding to the parameters 
                 need to be updated at this step. Added for block sampling
        ...
        if self.randomize_split:
            [model.random.shuffle(x) for x in inds]
        ...
        for branch_names_run, inds_run in self.gibbs_sampling_setup_iterator(
            ...
            ### NOTE: since inds_run are None, the inds going for proposal are just the branch inds
            ### For now we always keep gibbs_sampling = None 
            ...
            if not at_least_one_proposal:
                logger.warning('ATTENTION: No proposal, not performing within-model move!')
                continue

            # prepare accepted fraction
            accepted_here = np.zeros((ntemps, nwalkers), dtype=bool)
            for split in range(self.nsplits):
                ### NOTE: we do keep the slplits since they are used to run in parallel and should make things faster
                ...
                ### FIX: in principle we do not need gibbs_dim here!
                gibbs_ndim = 0
                ...
                q_tmp, factors = self.get_proposal(
                    s, c, inds_proposal, real_inds_subset, model.random, gibbs_ndim=gibbs_ndim
                )

                # account for gibbs sampling
                self.cleanup_proposals_gibbs(branch_names_run, inds_run, q_tmp, temp_coords)

                # setup supplimental information
                if state.supplimental is not None:
                    ###NOTE: in our case should always be None
                    # old TODO: should there be a copy?
                ...
                q, new_inds, new_branch_supps = self.ensure_ordering(
                    list(state.branches.keys()), q_tmp, new_inds, new_branch_supps
                )
                ...
                if np.any(np.isnan(logl)):
                    ...
                    logger.warning("Getting Nan in likelihood computation.")

                # catch and warn about -inf
                if np.any(np.isinf(logl)):
                    logl[np.isinf(logl)] = -1e300
                    logger.warning("Getting -inf in likelihood computation.")
