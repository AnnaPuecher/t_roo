...
def bilby_ifos_from_real_data(frame_files: dict,
                        channels: dict,
                        psd_files: dict,
                        duration: float,
                        start_time: float,
    ...
    if undefined_frames:
        raise ValueError(f"Frame files not provided for {undefined_frames}")

    undefined_ch = set(interferometers) - set(channels.keys())
    if undefined_ch:
        raise ValueError(f"Channel not provided for {undefined_ch}")

    undefined_psd = set(interferometers) - set(psd_files.keys())
    if undefined_psd:
        raise ValueError(f"PSD not provided for {undefined_psd}")


    for ifo in ifos:

        ifo.set_strain_data_from_frame_file(frame_file=frame_files[ifo.name],sampling_frequency=sampling_frequency, duration=duration, start_time=start_time,channel=channels[ifo.name])
