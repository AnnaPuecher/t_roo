...
class Backend(object):
    ...
    def _check_rj_accepted(self, rj_accepted, pre_steps):
        ...
        if self.rj and not pre_steps and rj_accepted is None:
            raise ValueError("inconsistent use of rj_accepted")
    ...
    def _check(
        self, state, accepted, pre_steps, rj_accepted=None, swaps_accepted=None,
    ):
        """Check all the information going in is okay."""
        self._check_blobs(state.blobs)
        self._check_rj_accepted(rj_accepted, pre_steps)
    ...
    def save_step(
        ...
        Args:
            ...
            pre_steps: True if we are in the within-model pre-rjmcmc-sampling stage,
                       otherwise False
