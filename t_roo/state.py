...
def initial_state_from_prior(priors,
    ...
    # assign each coordinate entry a number that tells us if it belongs to model1, model2, etc.
    assignments = np.random.randint(0, len(priors), size=(ntemps, nwalkers, 1))


    for j, model in enumerate(priors):
        
        # get initial state from prior
        coords[model] = priors[model].rvs(size=(ntemps, nwalkers, 1))
        inds[model] = (assignments == j)
